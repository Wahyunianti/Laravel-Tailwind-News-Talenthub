...
<div class="flex flex-col xl:flex-col h-auto w-full px-5 xl:px-20 gap-7 justify-around">
    <div class="flex flex-col">
        <div class="flex flex-row justify-between h-auto content-center mb-5">
            <p class="font-semibold text-lg">Edit Artikel</p>
            @if($errors->any())
        ...
        <div class="flex flex-col flex-grow-1 h-auto w-full border border-black rounded-md shadow-sm p-10 gap-5">
            <form class="form" id="forms" action="{{ route('artikel.update', ['id' => $atk->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="flex flex-col w-full">
                    <label for="" class="my-2">Judul Artikel</label>
                    <input type="text" name="judul" placeholder="judul artikel" value="{{$atk->judul}}" id="" class="rounded-md">
                    <label for="" class="my-2">Kategori Artikel</label>
                    <select name="kategoris_id" class="rounded-md cursor-pointer">
                        <option selected disabled>Pilih kategori</option>
                        @foreach ($ktg as $k)
                            <option value="{{ $k->id }}" {{ $atk->kategoris_id == $k->id ? 'selected' : '' }}>
                                {{ $k->nama }}
                            </option>
                        @endforeach
                    </select>
                    <label for="" class="my-2">Tag Artikel</label>
                    <input type="text" name="tags" placeholder="namatag, namatag, namatag" id="" class="rounded-md" value="@foreach($tg as $t){{$t->nama}}@if (!$loop->last),@endif @endforeach">
                    <label for="" class="my-2">Ganti Thumbnail</label>
                    <input type="file" class="border border-gray-600 rounded-md" name="foto" id="" class="rounded-md">
                </div>
                <div class="flex flex-col w-full">
                    <label for="" class="my-2">Konten</label>
                    <div class="rounded-md" id="editor" style="height: 500px;"> {!! $atk->konten !!}</div>
